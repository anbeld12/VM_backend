...
class ElColombianoSpider(RedisSpider):
    ...
    def parse(self, response):
        enlaces = response.css('h2 a::attr(href), h3 a::attr(href), .priority-content a::attr(href)').getall()
        for enlace in enlaces:
            url_completa = response.urljoin(enlace)
            yield scrapy.Request(url_completa, callback=self.parse_noticia)

    def parse_noticia(self, response):
        titulo = response.css('h1::text, .top-header h1::text').get()
        
        parrafos = response.css('.article-content p::text, .nota-interna p::text, article p::text').getall()
        if not parrafos:
