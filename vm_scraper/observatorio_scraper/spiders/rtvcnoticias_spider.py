...
class RTVCNoticiasSpider(RedisSpider):
    name = "rtvcnoticias"
    allowed_domains = ["rtvcnoticias.com"]
    start_urls = ["https://www.rtvcnoticias.com/nacion"]

    def parse(self, response):
        enlaces = response.css('h2 a::attr(href), h3 a::attr(href), article a::attr(href), .card-title a::attr(href)').getall()
    ...
    def parse_noticia(self, response):
        ...
        parrafos = response.css('article p::text, .article-content p::text, .entry-content p::text').getall()
        if not parrafos:
